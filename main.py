...
SYSTEM_LOGS = []
LIVE_COMMENTARY = "AI Trading Floor: Unrestricted Global Swarm Online."
...
async def autonomous_ai_brain():
    ...
    # Force loading of ALL UK and US instruments with absolute parsing robustness
    try:
        ...
        if res.status_code == 200:
            for inst in res.json():
                ...
                elif "l_EQ" in ticker or "L_EQ" in ticker or "GB_EQ" in ticker or "UK_EQ" in ticker:
    ...
    while True:
        try:
            ...
            # --- PHASE 1: EVALUATE SELLS (5% DAILY TARGET / 2% STRICT STOP LOSS) ---
            for t212_ticker, pos in owned_tickers.items():
                if t212_ticker in AI_SELL_COOLDOWN and (time.time() - AI_SELL_COOLDOWN[t212_ticker] < 60):
                    continue 
                ...
                if avg > 0:
                    ...
                    if ret_pct >= 5.0:
                        log_activity(f"🏆 5% TARGET ACHIEVED: Selling {t212_ticker} (+{ret_pct:.2f}%)", "success")
                        execute_live_order(t212_ticker, -qty)
                        AI_SELL_COOLDOWN[t212_ticker] = time.time()
                    elif ret_pct <= -2.0:
                        log_activity(f"✂️ RISK CONTROL: Cutting {t212_ticker} ({ret_pct:.2f}%)", "warning")
                        execute_live_order(t212_ticker, -qty)
                        AI_SELL_COOLDOWN[t212_ticker] = time.time()
            
            # --- PHASE 2: TOTAL FUND DEPLOYMENT (NO POSITION CAPS) ---
            if free_cash > TARGET_INVESTMENT_PER_TRADE:
                ...
                if available_pool:
                    # Sample massive batches of 300 stocks per cycle
                    ...
                    for t212_ticker in batch:
                        ...
                        if not data.empty and len(data) >= 3:
                            ...
                            if momentum > 0.002 and current_price > 0: # Ultra-sensitive momentum trigger
                                log_activity(f"🚀 DEPLOYING £500: {yf_ticker} ({momentum:+.3f}%)", "success")
                                ...
                                await asyncio.sleep(0.2) # High-speed execution loop
        ...
        await asyncio.sleep(1) # Continuous lightning-fast cycle
