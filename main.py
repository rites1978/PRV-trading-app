...
SYSTEM_LOGS = []
LIVE_COMMENTARY = "AI Trading Floor: News & Fundamental Intelligence Engine Active."
...
async def evaluate_fundamental_and_news_intel(yf_ticker: str) -> bool:
    try:
        ticker_obj = yf.Ticker(yf_ticker)
        news_items = ticker_obj.news
        if news_items:
            negative_keywords = ["lawsuit", "crash", "investigation", "fraud", "bankruptcy", "loss", "recall"]
            positive_keywords = ["growth", "surge", "beats", "profit", "expansion", "record", "upgrade"]
            
            pos_score = 0
            neg_score = 0
            for article in news_items[:3]:
                content = article.get("content", {})
                title = content.get("title", "").lower() if isinstance(content, dict) else str(article.get("title", "")).lower()
                for word in positive_keywords:
                    if word in title: pos_score += 1
                for word in negative_keywords:
                    if word in title: neg_score += 1
            if neg_score > pos_score:
                return False

        info = ticker_obj.info
        trailing_pe = info.get("trailingPE", None)
        if trailing_pe is not None and (trailing_pe < 0 or trailing_pe > 150):
            return False
...
async def autonomous_ai_brain():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            log_activity(f"Engine Ready: {len(ALL_UK_TICKERS)} UK & {len(ALL_US_TICKERS)} US Assets.", "success")
    except Exception as e:
        log_activity(f"Instrument load error: {str(e)}", "error")
    ...
    while True:
        try:
            ...
            for t212_ticker, pos in owned_tickers.items():
                ...
                if avg > 0:
                    ...
                    if ret_pct >= 0.4:
                        ...
                        log_activity(f"✂️ RISK CUT: Pruning {t212_ticker} ({ret_pct:.2f}%)", "warning")
                        execute_live_order(t212_ticker, -qty)
                        AI_SELL_COOLDOWN[t212_ticker] = time.time()
            ...
            if free_cash > 50.0:
                ...
                if available_pool:
                    batch = random.sample(available_pool, min(150, len(available_pool)))
                    ...
                    for t212_ticker in batch:
                        ...
                        if not data.empty and len(data) >= 5:
                            ...
                            avg_volume = sum(volumes[-5:]) / 5.0
                            latest_volume = volumes[-1]
                            momentum = ((closes[-1] - closes[-3]) / closes[-3]) * 100.0
                            
                            if momentum > 0.05 and latest_volume >= avg_volume and current_price > 0:
                                passes_intel = await evaluate_fundamental_and_news_intel(yf_ticker)
                                if not passes_intel:
                                    continue
                                ...
                                log_activity(f"📰 AI INTELLIGENCE BUY: {yf_ticker} (Momentum: {momentum:+.2f}%, Vol Confirmed)", "success")
                                execute_live_order(t212_ticker, qty)
                                AI_BUY_COOLDOWN[t212_ticker] = time.time()
...
from contextlib import asynccontextmanager
from db_manager import db
import databento as db_client_sdk
import yfinance as yf
import warnings
warnings.filterwarnings("ignore")
...
SYSTEM_LOGS = []
LIVE_COMMENTARY = "AI Trading Floor: Hybrid Databento / Fallback Engine Active."
...
# Initialize Databento with a credit conservation tracker
DATABENTO_API_KEY = os.getenv("DATABENTO_API_KEY", "").strip()
db_historical = db_client_sdk.Historical(DATABENTO_API_KEY) if DATABENTO_API_KEY else None
DATABENTO_CREDITS_EXHAUSTED = False
...
async def get_market_signal(symbol: str, yf_fallback_symbol: str) -> dict:
    global DATABENTO_CREDITS_EXHAUSTED
    
    # 1. Try Databento first if credits are available
    if db_historical and not DATABENTO_CREDITS_EXHAUSTED:
        try:
            data = db_historical.timeseries.get_range(
                dataset="XNAS.ITCH",
                schema="ohlcv-1m",
                symbols=[symbol],
            ...
            if not df.empty and len(df) >= 2:
                ...
                momentum = ((closes[-1] - closes[-2]) / closes[-2]) * 100.0
                return {"valid": momentum > 0.01, "momentum": momentum, "source": "Databento"}
        except Exception as e:
            if "credit" in str(e).lower() or "limit" in str(e).lower() or "balance" in str(e).lower():
                DATABENTO_CREDITS_EXHAUSTED = True
                log_activity("⚠️ Databento credits exhausted. Switching to regular mode (Yahoo Finance)...", "warning")
            pass

    # 2. Regular Mode Fallback (Yahoo Finance)
    try:
        data = yf.download(yf_fallback_symbol, period="1d", interval="1m", progress=False)
        if not data.empty and len(data) >= 3:
            closes = [float(x) for x in data['Close'].values.flatten()]
            momentum = ((closes[-1] - closes[-3]) / closes[-3]) * 100.0
            return {"valid": momentum > 0.02, "momentum": momentum, "source": "Standard"}
    except Exception:
        pass
        
    return {"valid": False, "momentum": 0.0, "source": "None"}
...
async def autonomous_ai_brain():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            log_activity(f"Engine Synced: {len(ALL_UK_TICKERS)} UK & {len(ALL_US_TICKERS)} US Assets Loaded.", "success")
    except Exception as e:
        log_activity(f"Instrument load error: {str(e)}", "error")
    ...
    while True:
        try:
            ...
            if free_cash > 50.0:
                ...
                if available_pool:
                    batch = random.sample(available_pool, min(100, len(available_pool)))
                    ...
                    for t212_ticker in batch:
                        ...
                        if "l_EQ" in t212_ticker or "L_EQ" in t212_ticker:
                            clean_symbol = t212_ticker.replace("l_EQ", "").replace("L_EQ", "")
                            yf_symbol = clean_symbol + ".L"
                        elif t212_ticker.endswith("_US_EQ"):
                            clean_symbol = t212_ticker.replace("_US_EQ", "")
                            yf_symbol = clean_symbol
                        else:
                            continue
                        
                        # Fetch signal with automatic conservation & fallback
                        signal = await get_market_signal(clean_symbol, yf_symbol)
                        
                        if signal["valid"]:
                            current_price = float(owned_tickers.get(t212_ticker, {}).get("currentPrice", 100.0))
                            target_spend = min(500.0, free_cash)
                            ...
                            log_activity(f"🚀 AI STRIKE [{signal['source']}]: {clean_symbol} ({signal['momentum']:+.2f}%)", "success")
                            execute_live_order(t212_ticker, qty)
                            AI_BUY_COOLDOWN[t212_ticker] = time.time()
                            await asyncio.sleep(0.3)
