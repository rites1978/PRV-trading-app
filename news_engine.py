...
SYSTEM_LOGS = []
LIVE_COMMENTARY = "AI Trading Floor: News-Driven & Fundamental Intelligence Engine Online."
...
async def evaluate_fundamental_and_news_intel(yf_ticker: str) -> bool:
    """Uses fundamental data and news sentiment to validate trading decisions."""
    try:
        ticker_obj = yf.Ticker(yf_ticker)
        
        # 1. News Sentiment Check
        news_items = ticker_obj.news
        if news_items:
            # Analyze recent headlines for negative risk keywords
            ...
            for article in news_items[:3]:
                # Access news dictionary format safely
            ...
            # If news is heavily negative, reject the trade
            if neg_score > pos_score:
                return False

        # 2. Fundamental Valuation Guardrail
        info = ticker_obj.info
        trailing_pe = info.get("trailingPE", None)
        if trailing_pe is not None and (trailing_pe < 0 or trailing_pe > 150):
            # Avoid extreme overvaluation or failing companies
            return False

        return True
    except Exception:
        # If news/fundamentals lookup encounters rate limits, default to technical validation
        return True
...
async def autonomous_ai_brain():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            log_activity(f"Intelligence Engine Ready: {len(ALL_UK_TICKERS)} UK & {len(ALL_US_TICKERS)} US Assets.", "success")
    except Exception as e:
        ...
        try:
            ...
            for t212_ticker, pos in owned_tickers.items():
                ...
                if avg > 0:
                    ...
                    if ret_pct >= 0.4: # Slightly elevated profit target to beat spread friction
            ...
            # --- PHASE 2: NEWS & FUNDAMENTALS DRIVEN CAPITAL DEPLOYMENT ---
            if free_cash > 50.0:
                available_pool = []
...
git add main.py
git commit -m "Upgrade algorithm to incorporate news sentiment screening, fundamental checks, and volume-weighted momentum"
