...
class ForwardTestIntegrityGuard:
    ...
    def validate_pre_flight_compliance(
        ...
        pos_pct = getattr(settings, "DEFAULT_POSITION_PCT", 0.0553)
        ...
        if order_cost_gbp > max_allowed_cost:
            return False, f"COMPLIANCE REJECTION: Order cost (£{order_cost_gbp:.2f}) exceeds 5.53% sizing limit (£{max_allowed_cost:.2f})!", audit_log
