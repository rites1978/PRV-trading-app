...
class TradingSettings(BaseModel):
    ...
    TRADING212_API_SECRET: str = Field(default_factory=lambda: os.getenv("TRADING212_API_SECRET", ""))
    
    # Capital Management
    STARTING_CAPITAL: float = 50000.0
    MIN_CASH_BUFFER_PCT: float = 0.05  # 5% cash buffer always held
    MAX_DEPLOYMENT_NEUTRAL: float = 0.20
    MAX_DEPLOYMENT_STRONG: float = 0.50
    MAX_DEPLOYMENT_EXCEPTIONAL: float = 0.80
    
    # Risk Limits
    MAX_DAILY_DRAWDOWN_PCT: float = 0.05  # 5% hard daily circuit breaker
    MAX_POSITION_SIZE_PCT: float = 0.08    # Max 8% of Core Capital per position
    MAX_SECTOR_EXPOSURE_PCT: float = 0.25  # Max 25% exposure per sector
    ...
    MIN_REWARD_RISK_RATIO: float = 3.0     # Strict 3:1 Reward to Risk
    
    # AI Scoring & Execution Filters
    MIN_CONFIDENCE_THRESHOLD: float = 80.0 # Strict confidence threshold
